...
export class AgentOrchestrator extends EventEmitter {
    ...
    try {
      ...
      if (response.streaming) {
        for await (const chunk of response.content) {
          this.emit('planChunk', chunk);
        }
      }

      const planContent = response.streaming ? '' : response.content as string;
  ...
  private async executeActionPhase(context: string, taskState: TaskState): Promise<TaskStatus> {
    try {
      ...
      if (response.streaming) {
        for await (const chunk of response.content) {
          this.emit('actionChunk', chunk);
        }
      }

      const actionContent = response.streaming ? '' : response.content as string;
      ...
      taskState.actorSummary = response.summary || '';
  ...
  private async executeEvaluationPhase(context: string, taskState: TaskState): Promise<{ status: TaskStatus; nextContext: string }> {
    try {
      ...
      if (response.streaming) {
        for await (const chunk of response.content) {
          this.emit('evaluationChunk', chunk);
        }
      }

      const evalContent = response.streaming ? '' : response.content as string;
      const decision = this.parseEvaluationDecision(evalContent);
      
      taskState.lastEvalDecision = decision;
      taskState.evaluatorSummary = response.summary || '';
