...
from termaite.core.project_initialization import (
    ProjectInitializationTask,
    create_project_initialization_task,
)
from termaite.llm.parsers import (
...
class TestProjectInitializationTask:
    ...
    def test_execute_creates_termaite_directory(self):
        ...
        mock_task_handler.config = {
            "plan_prompt": "test",
            "action_prompt": "test",
            "evaluate_prompt": "test",
        }
        mock_task_handler.config_manager.get_command_maps.return_value = ({}, {})
        mock_task_handler.handle_task.return_value = True
        mock_task_handler.llm_client.send_request = Mock(
            return_value="No files generated"
        )
...
"""
            ...
            file_paths = [
                str(termaite_dir / f)
                for f in ["PLANNER.md", "ACTOR.md", "EVALUATOR.md"]
            ]
    ...
    def test_validate_generated_prompt_files_invalid(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (termaite_dir / "ACTOR.md").write_text(
                "Long enough content but missing the required Project-Specific section that we need."
            )
